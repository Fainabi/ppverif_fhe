...
fn hint_benchmark(c: &mut Criterion) {
    ...
    ] 
    {
        ...
        let mut server= Server::new(ip_param, DEFAULT_BLIND_ROTATION_PARAMETER, glwe_pk);

        let mut rng = thread_rng();
        let mut features_to_verif = (0..ip_param.polynomial_size.0)
            .map(|_| (rng.next_u32() % 1024) as f32 / 1024.0)
            .collect::<Vec<_>>();
        let mut features_to_enroll = (0..ip_param.polynomial_size.0)
            .map(|_| (rng.next_u32() % 1024) as f32 / 1024.0)
            .collect::<Vec<_>>();
...
criterion_group!(benches, hint_benchmark);
